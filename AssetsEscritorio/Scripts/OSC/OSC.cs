...
public class UDPPacketIO
{
	...
	public void setRemoteHostName(string s){
   	 remoteHostName = s;
   	 Debug.Log("el nuevo host es: "+remoteHostName);
    }
...
//{
  ...
  public class OSC : MonoBehaviour
  {
	...
	public void reiniciar(string salidaIP){
		outIP = salidaIP;
		OscPacketIO.setRemoteHostName(outIP);
	}
	...
	void Awake() {
   	 ...
   	 GameObject[] obj = GameObject.FindGameObjectsWithTag("ManagerOSCUnico");
   	 //if (obj != null ){
   	 if(obj.Length<2 && !created){
   		 Debug.Log("---------------------------Cree un OSC:------------------------------");
